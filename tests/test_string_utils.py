...
def test_find_palindromes_with_palindromes():
    result = StringProcessor.find_palindromes("madam level noon hello")
    assert sorted(result) == ["level", "madam", "noon"]

def test_find_palindromes_case_insensitive():
    result = StringProcessor.find_palindromes("Madam Level")
    assert sorted(result) == ["level", "madam"]

def test_truncate_text_empty():
    assert StringProcessor.truncate_text("", 5) == ""
