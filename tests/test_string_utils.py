...
def test_find_palindromes_multiple_palindromes():
    assert sorted(StringProcessor.find_palindromes("madam saw level noon")) == ["level", "madam", "noon"]

def test_find_palindromes_case_insensitive():
    assert StringProcessor.find_palindromes("Level Madam") == ["level", "madam"]

def test_find_palindromes_with_punctuation():
    assert StringProcessor.find_palindromes("Level! Madam, noon.") == ["level", "madam", "noon"]
...
def test_truncate_text_custom_suffix():
    assert StringProcessor.truncate_text("hello world", 8, "...!") == "hell...!"
