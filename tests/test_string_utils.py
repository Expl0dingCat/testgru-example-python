...
def test_find_palindromes_with_palindromes():
    result = StringProcessor.find_palindromes("level noon deed hello")
    assert sorted(result) == sorted(["level", "noon", "deed"])

def test_find_palindromes_case_insensitive():
    result = StringProcessor.find_palindromes("Level Noon Deed")
    assert sorted(result) == sorted(["level", "noon", "deed"])
